...
BEGIN
  ...
  IF TG_OP IN ('INSERT') THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Trade must be sequential in terms of seq and timestamp';
    END IF;
  END IF;
...
BEGIN
  -- YOUR IMPLEMENTATION GOES HERE >>>
  -- check when we are doing an insert or update on the Broker table
  IF TG_TABLE_NAME = 'broker' AND EXISTS (SELECT 1 FROM Owns WHERE pid = NEW.pid) THEN
    RAISE EXCEPTION 'Broker cannot own its own account';
  END IF;

  -- check when we are doing an insert or update on the Owns table
  IF TG_TABLE_NAME = 'owns' AND EXISTS (SELECT 1 FROM Broker WHERE pid = NEW.pid) THEN
    RAISE EXCEPTION 'Broker cannot own its own account';
  END IF;
